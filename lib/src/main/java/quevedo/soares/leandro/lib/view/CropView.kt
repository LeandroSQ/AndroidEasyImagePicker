...
class CropView : FrameLayout {
	...
	private fun onInitialized() {
		...
		this.cropRectMaskPaint.alpha = 100
	...
	private fun handleDrag(position: PointF) {
		...
		when (this.dragType!!) {
			...
			DragType.Bottom -> {
				...
				} else if (hasMinimumSet && cropRect.height() > rectMaxHeight) {
		...
		image?.let {
			...
			if (it.width > it.height) {
				...
				if (it.height >= this.height) {
					...
					// Just center the image vertically
					imageFinalH = it.height.toFloat()
					imageFinalY = max(0f, (this.height - imageFinalH) / 2f) //(this.height - it.height) / 2f + (it.height / 4f)
			...
			if (hasMaximumSet && (cropRect.width() > rectMaxWidth || cropRect.height() > rectMaxHeight)) {
				val x = imageRect.left + imageRect.width() / 2f - rectMaxWidth
				val y = imageRect.top + imageRect.height() / 2f - rectMaxHeight
